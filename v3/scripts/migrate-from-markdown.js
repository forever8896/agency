...
import { initializeDatabase, closeDatabase, getDatabase, taskQueries, handoffQueries, eventQueries, } from '../src/db/database.js';
...
function parseHandoffs(handoffsDir) {
    const handoffs = [];
    try {
        ...
        for (const file of files) {
            ...
            if (name.includes('bug'))
                ...
                type = 'review-request';
            else if (name.includes('blocker') || name.includes('blocked'))
                type = 'blocker';
            else if (name.includes('discovery'))
                type = 'design-doc';
            handoffs.push({
...
async function main() {
    ...
    const existingCount = db.prepare('SELECT COUNT(*) as count FROM tasks').get();
    ...
    const existingHandoffs = db.prepare('SELECT COUNT(*) as count FROM handoffs').get();
  ...
  `).all();
    ...
    const totalHandoffs = db.prepare('SELECT COUNT(*) as count FROM handoffs').get();
